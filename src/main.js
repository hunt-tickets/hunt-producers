...
class ProducerForm {
  constructor() {
    this.form = document.getElementById('producer-form')
  ...
  isValidInput(value, type) {
    ...
    const patterns = {
      businessName: /^[a-zA-ZÀ-ÿ\u00C0-\u017F\s'-0-9]{2,100}$/,
  ...
  setupPaymentMethodFields() {
    ...
    setTimeout(() => {
      ...
      if (!brebField || !bankFields || paymentMethods.length === 0) {
        return
      }
      ...
      paymentMethods.forEach(radio => {
        radio.addEventListener('change', (e) => {
          this.handlePaymentMethodChange(e.target.value, brebField, bankFields, bankFieldIds)
        })
      })
      ...
      refundLabels.forEach(label => {
        label.addEventListener('click', (e) => {
          ...
          if (radio && !radio.checked) {
            radio.checked = true
            this.handlePaymentMethodChange(radio.value, brebField, bankFields, bankFieldIds)
          }
        })
      })
    }, 100)
  ...
  handlePaymentMethodChange(value, brebField, bankFields, bankFieldIds) {
  ...
  validateField(field) {
    ...
    if (fieldName === 'businessName' && value.length < 2) {
      this.showError(fieldName, this.t('business-name-length'))
      return false
    }
    
    if (fieldName === 'contactName' && value.length < 2) {
      this.showError(fieldName, this.t('contact-name-length'))
      return false
    }
    ...
    if (fieldName === 'taxId' && !/^[A-Za-z0-9\-]{6,50}$/.test(value)) {
      this.showError(fieldName, this.t('invalid-tax-id'))
      return false
    }
    
    if (fieldName === 'businessDescription' && value.length < 10) {
      this.showError(fieldName, this.t('business-description-length'))
      return false
    }
  ...
  validateForm() {
    ...
    if (!paymentMethod) {
      this.showError('paymentMethod', this.t('payment-method-error'))
      isValid = false
    }
...
document.addEventListener('DOMContentLoaded', () => {
  // Initialize form
  const producerForm = new ProducerForm()
